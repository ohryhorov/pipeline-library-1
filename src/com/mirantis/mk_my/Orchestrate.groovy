...
def validateFoundationInfra(master, extra_tgt = null) {
    ...
    salt.cmdRun(master, 'I@salt:master' ,'salt-key', true, null, true, [], extra_tgt)
    salt.runSaltProcessStep(master, 'I@salt:minion', 'test.version', [], null, true, -1, null, extra_tgt)
    salt.cmdRun(master, 'I@salt:master' ,'reclass-salt --top', true, null, true, [], extra_tgt)
    salt.runSaltProcessStep(master, 'I@reclass:storage', 'reclass.inventory', [], null, true, -1, null, extra_tgt)
    salt.runSaltProcessStep(master, 'I@salt:minion', 'state.show_top', [], null, true, -1, null, extra_tgt)
}

def installFoundationInfra(master, staticMgmtNet=false, extra_target='') {
    def salt = new com.mirantis.mk.Salt()
    ...
    salt.enforceState(master, "I@salt:master ${extra_target}", ['reclass'])

    salt.enforceState(master, "I@salt:master ${extra_target}", ['linux.system'])
    salt.enforceState(master, "I@salt:master ${extra_target}", ['salt.master'], true, false, null, false, 120, 2)
    salt.fullRefresh(master, "*")

    salt.enforceState(master, "I@salt:master ${extra_target}", ['salt.minion'], true, false, null, false, 60, 2)
    salt.enforceState(master, "I@salt:master ${extra_target}", ['salt.minion'])
    salt.fullRefresh(master, "*")
    ...
    salt.enforceState(master, "I@linux:network:interface ${extra_target}", ['linux.network.interface'])
    sleep(5)
    salt.enforceState(master, "I@linux:system ${extra_target}", ['linux', 'openssh', 'ntp', 'rsyslog'])
    ...
    if (salt.testTarget(master, "I@iptables:service ${extra_target}")) {
        salt.enforceState(master, "I@iptables:service ${extra_target}", 'iptables')
    }

    // Install and configure logrotate
    if (salt.testTarget(master, "I@logrotate:server ${extra_target}")) {
        salt.enforceState(master, "I@logrotate:server ${extra_target}", 'logrotate')
    }
}
...
def installInfraKvm(master, extra_tgt = null) {
    ...
    salt.enforceState(master, 'I@salt:control', ['salt.minion'], true, false, null, false, 60, 2, true, [], 10, extra_tgt)
    salt.enforceState(master, 'I@salt:control', ['linux.system', 'linux.network', 'ntp', 'rsyslog'], true, true, null, false, -1, -1, true, [], 10, extra_tgt)
    salt.enforceState(master, 'I@salt:control', 'libvirt', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
    salt.enforceState(master, 'I@salt:control', 'salt.control', true, true, null, false, -1, -1, true, [], 10, extra_tgt)

    common.infoMsg("Building minions list...")
    if (salt.testTarget(master, infra_compound, extra_tgt)) {
        // Gathering minions
        for ( infra_node in salt.getMinionsSorted(master, infra_compound, extra_tgt) ) {
            def pillar = salt.getPillar(master, infra_node, 'salt:control:cluster', extra_tgt)
    ...
    timeout(time: wait_timeout, unit: 'MINUTES') {
        salt.minionsPresentFromList(master, 'I@salt:master', minions, true, null, true, retries, 1, extra_tgt)
    }

    common.infoMsg('Waiting for minions to respond')
    timeout(time: wait_timeout, unit: 'MINUTES') {
        salt.minionsReachable(master, 'I@salt:master', minions_compound, extra_tgt)
    }
...
def installInfra(master, extra_tgt = null) {
    ...
    if (salt.testTarget(master, 'I@glusterfs:server', extra_tgt)) {
        salt.enforceState(master, 'I@glusterfs:server', 'glusterfs.server.service', true, true, null, false, -1, -1, true, [], 10, extra_tgt)

        salt.enforceState(master, 'I@glusterfs:server and *01*', 'glusterfs.server.setup', true, true, null, false, -1, 5, true, [], 10, extra_tgt)
        sleep(10)
        salt.cmdRun(master, 'I@glusterfs:server', "gluster peer status; gluster volume status", true, null, true, [], extra_tgt)
    }

    // Ensure glusterfs clusters is ready
    if (salt.testTarget(master, 'I@glusterfs:client', extra_tgt)) {
        salt.enforceState(master, 'I@glusterfs:client', 'glusterfs.client', true, true, null, false, -1, 2, true, [], 10, extra_tgt)
    }

    // Install galera
    if (salt.testTarget(master, 'I@galera:master', extra_tgt) || salt.testTarget(master, 'I@galera:slave', extra_tgt)) {
        salt.enforceState(master, 'I@galera:master', 'galera', true, true, null, false, -1, 2, true, [], 10, extra_tgt)
        salt.enforceState(master, 'I@galera:slave', 'galera', true, true, null, false, -1, 2, true, [], 10, extra_tgt)

        // Check galera status
        salt.runSaltProcessStep(master, 'I@galera:master', 'mysql.status', [], null, true, -1, null, extra_tgt)
        salt.runSaltProcessStep(master, 'I@galera:slave', 'mysql.status', [], null, true, -1, null, extra_tgt)
    // If galera is not enabled check if we need to install mysql:server
    } else if (salt.testTarget(master, 'I@mysql:server', extra_tgt)){
        salt.enforceState(master, 'I@mysql:server', 'mysql.server', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        if (salt.testTarget(master, 'I@mysql:client', extra_tgt)){
            salt.enforceState(master, 'I@mysql:client', 'mysql.client', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
    }
...
def installBackup(master, component='common', extra_tgt = null) {
    ...
    if (component == 'common') {
        ...
        if (salt.testTarget(master, 'I@backupninja:client', extra_tgt)) {
            salt.enforceState(master, 'I@backupninja:client', 'salt.minion.grains', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
            salt.runSaltProcessStep(master, 'I@backupninja:client', 'saltutil.sync_grains', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@backupninja:client', 'mine.flush', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@backupninja:client', 'mine.update', [], null, true, -1, null, extra_tgt)
            salt.enforceState(master, 'I@backupninja:client', 'backupninja', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
        if (salt.testTarget(master, 'I@backupninja:server', extra_tgt)) {
            salt.enforceState(master, 'I@backupninja:server', 'salt.minion.grains', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
            salt.enforceState(master, 'I@backupninja:server', 'backupninja', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
    } else if (component == 'mysql') {
        // Install Xtrabackup
        if (salt.testTarget(master, 'I@xtrabackup:client', extra_tgt)) {
            salt.enforceState(master, 'I@xtrabackup:client', 'salt.minion.grains', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
            salt.runSaltProcessStep(master, 'I@xtrabackup:client', 'saltutil.sync_grains', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@xtrabackup:client', 'mine.flush', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@xtrabackup:client', 'mine.update', [], null, true, -1, null, extra_tgt)
            salt.enforceState(master, 'I@xtrabackup:client', 'xtrabackup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
        if (salt.testTarget(master, 'I@xtrabackup:server', extra_tgt)) {
            salt.enforceState(master, 'I@xtrabackup:server', 'xtrabackup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
    } else if (component == 'contrail') {

        // Install Cassandra backup
        if (salt.testTarget(master, 'I@cassandra:backup:client', extra_tgt)) {
            salt.enforceState(master, 'I@cassandra:backup:client', 'salt.minion.grains', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
            salt.runSaltProcessStep(master, 'I@cassandra:backup:client', 'saltutil.sync_grains', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@cassandra:backup:client', 'mine.flush', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@cassandra:backup:client', 'mine.update', [], null, true, -1, null, extra_tgt)
            salt.enforceState(master, 'I@cassandra:backup:client', 'cassandra.backup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
        if (salt.testTarget(master, 'I@cassandra:backup:server', extra_tgt)) {
            salt.enforceState(master, 'I@cassandra:backup:server', 'cassandra.backup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
        // Install Zookeeper backup
        if (salt.testTarget(master, 'I@zookeeper:backup:client', extra_tgt)) {
            salt.enforceState(master, 'I@zookeeper:backup:client', 'salt.minion.grains', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
            salt.runSaltProcessStep(master, 'I@zookeeper:backup:client', 'saltutil.sync_grains', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@zookeeper:backup:client', 'mine.flush', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@zookeeper:backup:client', 'mine.update', [], null, true, -1, null, extra_tgt)
            salt.enforceState(master, 'I@zookeeper:backup:client', 'zookeeper.backup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
        if (salt.testTarget(master, 'I@zookeeper:backup:server', extra_tgt)) {
            salt.enforceState(master, 'I@zookeeper:backup:server', 'zookeeper.backup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
    } else if (component == 'ceph') {
        // Install Ceph backup
        if (salt.testTarget(master, 'I@ceph:backup:client', extra_tgt)) {
            salt.enforceState(master, 'I@ceph:backup:client', 'salt.minion.grains', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
            salt.runSaltProcessStep(master, 'I@ceph:backup:client', 'saltutil.sync_grains', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@ceph:backup:client', 'mine.flush', [], null, true, -1, null, extra_tgt)
            salt.runSaltProcessStep(master, 'I@ceph:backup:client', 'mine.update', [], null, true, -1, null, extra_tgt)
            salt.enforceState(master, 'I@ceph:backup:client', 'ceph.backup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
        if (salt.testTarget(master, 'I@ceph:backup:server', extra_tgt)) {
            salt.enforceState(master, 'I@ceph:backup:server', 'ceph.backup', true, true, null, false, -1, -1, true, [], 10, extra_tgt)
        }
    }
